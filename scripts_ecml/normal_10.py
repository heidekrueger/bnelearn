...
regret_batch_size = 2**12
regret_grid_size = 2**10
## ES
learner_hyperparams = {
    'population_size': 64,
    'sigma': 1.,
...
def training_loop(self, writer, e):
    ...
    if e % self._logging_options['plot_epoch'] == 0:
        ...
        ## regret
        grid = torch.linspace(plot_xmin,plot_xmax, steps = regret_grid_size).unsqueeze(-1)

        bid_profile = torch.empty(regret_batch_size, n_players, 1, device = self.env.agents[0].valuations.device)
        for i,a in enumerate(self.env.agents):
            bid_profile[:,i,:] = a.get_action()[:regret_batch_size]
        # assert first player has (near) zero regret
        regret = metrics.ex_interim_regret(self.mechanism, bid_profile, player_position = 0,
                                        agent_valuation = self.env.agents[0].valuations[:regret_batch_size],
                                        agent_bid_actual = self.env.agents[0].get_action()[:regret_batch_size],
                                        grid = grid
                                        )
        mean_regret = regret.mean()
        max_regret = regret.max()
        writer.add_scalar('eval/regret_mean', mean_regret, e)
        writer.add_scalar('eval/regret_max', max_regret, e)
