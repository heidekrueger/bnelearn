...
class CombinatorialExperiment(Experiment, ABC):
    def __init__(self, mechanism_type, gpu_config: GPUController, logger: Logger, l_config: LearningConfiguration, model_sharing = True):
    ...
    def strat_to_bidder(self, strategy, player_position = 0, batch_size = None):
        return Bidder.uniform(self.u_lo, self.u_hi[player_position], strategy, player_position=player_position, batch_size = batch_size)
...
class LLGExperiment(CombinatorialExperiment):
    ...
    def setup_bidders(self):
        ...
        bidder_l2 = self.strat_to_bidder(model_l1, player_position=1, batch_size = self.l_config.batch_size) \
                        if self.model_sharing else self.strat_to_bidder(model_l2, player_position=1, batch_size = self.l_config.batch_size)
    ...
    def optimal_bid(self, valuation, player_position):
        ...
        if self.mechanism_type in ['vcg', 'proxy', 'nearest_zero', 'nearest_bid', 'nearest_vcg'] and player_position == 2:
            return valuation
        # local bidders:
        ...
        if self.mechanism_type == 'nearest_vcg':
            bid_if_positive = 2. / (2. + self.gamma) * (valuation - (3. - np.sqrt(9 - (1. - self.gamma)**2)) / (1. - self.gamma) )
            return torch.max(torch.zeros_like(valuation), bid_if_positive)
    ...
    def training_loop(self, epoch):
        ...
        self.logger.log_training_iteration(prev_params=prev_params[0], epoch=epoch, bne_env=self.bne_env,
                                           ...
                                           eval_batch_size=self.l_config.eval_batch_size, bne_utility=self.bne_utility[0],
