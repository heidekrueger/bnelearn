...
class OptimizableExperiment:
    ...
    def optimize(self, init_points: int = 3, minimize: bool = True):
        optimizer = BayesianOptimization(f=self._get_optimizable_function(minimize=minimize), pbounds=self.hp_bounds, verbose=2, random_state=1)        
    ...
    def _get_optimizable_function(self, minimize: bool):
        #If we want hp's to be dynamically determined from the given bounds, we would need to create this method (its signature) dynamically
        def optimizable_function(lr: float = 1e-3) -> float:            
...
try:
    ...
    experiment_config, experiment_class = ConfigurationManager(experiment_type='single_item_uniform_symmetric', n_runs=3, n_epochs=500) \
    ...
    experiment = OptimizableExperiment(experiment_class=experiment_class, experiment_config=experiment_config, hp_bounds={'lr': (0.0001, 0.01)},
                                        n_runs_budget=20)
    experiment.optimize()
