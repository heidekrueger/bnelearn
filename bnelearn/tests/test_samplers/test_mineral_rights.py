...
def test_mineral_rights(n_players, valuation_size, u_lo, u_hi):
    ...
    #shorthand for batch_sizes
    outer = conditioned_observation.shape[0]
    inner = conditional_inner_batch_size

    for i in range(n_players):

        cv, co = s.draw_conditional_profiles(i, conditioned_observation, conditional_inner_batch_size)

        cv_means =cv.view(outer, inner, n_players, valuation_size).mean(dim=1)
        co_means =co.view(outer, inner, n_players, valuation_size).mean(dim=1)

        # Note: for 0<obs<min(2*u_lo,u_hi), observations could have been generated by any valuation
        indices = [0,1,2] if u_hi <= 2*u_lo else [0,1]
        assert torch.allclose(cv_means[[indices]] - cv_means[0],
                              torch.zeros_like(cv_means[[indices]]),
                              atol=0.01), \
            "Low observations should ne noninformative about v."
        assert torch.all(cv_means[0].less_equal(cv_means[3])), \
            "Larger observations should yield larger valuations!"
        assert torch.all(cv_means[3].less_equal(cv_means[4])), \
            "Larger observations should yield larger valuations!"
        assert torch.allclose(cv_means[4] - u_hi, torch.zeros_like(cv_means[4])), \
            "Highest observation can only be caused by highest valuation."
