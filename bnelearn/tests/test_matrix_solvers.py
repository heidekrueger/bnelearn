...
def run_fudenberg_smooth_fictitious_play(n_players, game, initial_beliefs=None, iterations=1000):
    ...
    return env.solve_with_fudenberg_smooth_fictitious_play(
        dev = device,
        initial_beliefs=initial_beliefs,
        iterations = iterations
    )
...
def test_fictitious_play_with_BattleOfTheSexes_Mod_3x2():
    ...
    assert torch.allclose(strategy[0], torch.tensor([1.,0,0], device = device),atol = 0.01)
...
def test_smooth_fictitious_play_with_PrisonersDilemma_2x2():
...
def test_smooth_fictitious_play_with_BattleOfTheSexes_2x2():
    ...
    strategy = run_smooth_fictitious_play(
...
def test_smooth_fictitious_play_with_PaulTestGame_2x2x2():
    expected_valuation = run_smooth_fictitious_play(
        3, PaulTestGame(cuda = cuda),
    ...
    assert torch.allclose(expected_valuation[0][-1], torch.tensor([1.98,0.98], device = device),atol = 0.01)
    assert torch.allclose(expected_valuation[1][-1], torch.tensor([1.3,4.89], device = device),atol = 0.01)
...
def test_k_smooth_fictitious_play_with_BattleOfTheSexes_2x2():
    ...
    # Test pure Nash
    _, _, strategy = run_k_smooth_fictitious_play(
        2,
        BattleOfTheSexes(cuda = cuda),
        initial_beliefs = [torch.tensor([0.8,0.2], device = device),
                           torch.tensor([0.8,0.2], device = device)],
...
def test_k_smooth_fictitious_play_with_PaulTestGame_2x2x2():
    _, expected_valuation, _ = run_k_smooth_fictitious_play(
        3,
...
def test_fudenberg_smooth_fictitious_play_with_MatchingPennies_2x2():
    ...
    _, _, strategy = run_fudenberg_smooth_fictitious_play(
        2,
        BattleOfTheSexes(cuda = cuda),
        initial_beliefs = [torch.tensor([600,400], device = device),
                           torch.tensor([400,600], device = device)],
        iterations = 3000
        )

    assert torch.allclose(strategy[0], torch.tensor([0.6,0.4], device = device),atol = 0.01), "Invalid strategy {} for player {}".format(strategy[0],0)
    assert torch.allclose(strategy[1], torch.tensor([0.4,0.6], device = device),atol = 0.01), "Invalid strategy {} for player {}".format(strategy[1],1)
    '''
