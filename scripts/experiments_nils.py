...
for vals in product(*hyperparams.values()):
    ...
    def plot_bid_function_3d(writer, e, save_fig_to_disc=False):
        ...
        for agent_idx in range(n_players):
            ...
            strategy = bidders[agent_idx].strategy.play(plot_valuations[:,agent_idx,:])
        ...
        for agent_idx in range(n_players):
            for item_idx in range(n_items):
                ...
                ax.set_title('agent {} biddings for item {}'.format(agent_idx, item_idx))
                ax.view_init(20, -135)
